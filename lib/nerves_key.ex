defmodule NervesKey do
  ...
  * :years_valid - how many years this key is valid for
  ...
  @spec generate_csr( ATECC508A.Transport.t(), :public_key.ec_public_key() ) :: X509.Certificate.t()
  def generate_csr(transport, atecc508a_public_key) do
    check_time()
    ...
    csr = X509.CSR.new(key, subject_rdn, public_key: atecc508a_public_key,
                        extension_request: [
  ...
  @spec get_csr_signed(
          ...
          String.t(),
          Integer.t()
        ) :: X509.Certificate.t() | String.t()
  def get_csr_signed(transport, csr_server_ip \\ "192.168.43.180", csr_server_port \\ 9000, route \\ "/csr/sign") do
    check_time()
    ...
    b64_device_sn = Base.encode64(device_sn)
    body = Poison.encode!( %{"csr_str": csr_pem, "device_sn": b64_device_sn, "manufacturer_sn": manufacturer_sn} )
  ...
  def check_pkis_issuer(device_cert, signer_cert) do
    :public_key.pkix_is_issuer(device_cert, signer_cert)
  end

  def check_signature(device_cert, signer_cert) do
    signer_public_key = X509.Certificate.public_key(signer_cert)
    :public_key.pkix_verify(X509.Certificate.to_der(device_cert), signer_public_key)
  end
